...
public class DonViVCRepo {
    ...
    public boolean insertFl(DonViVanChuyen dvvc) {

        try {

            String sql = "Insert into DonViVanChuyen(ten,phamViVanChuyen,trangThai,hotLine) values(?,?,1,?)";
            PreparedStatement ps = conn.prepareStatement(sql);

           
            ps.setString(1, dvvc.getTen());
            ps.setString(2, dvvc.getPhamViVC());
            ps.setString(3, dvvc.getHotLine());
    ...
    public List<DonViVanChuyen> all() {
     ...
     public List<DonViVanChuyen> DVVCDangHD() {

        List<DonViVanChuyen> ds = new ArrayList<>();
        try {

            String sql = "Select * from DonViVanChuyen where trangThai =1";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.execute();
            ResultSet rs = ps.getResultSet();
            while (rs.next()) {

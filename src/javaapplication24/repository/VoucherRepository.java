...
public class VoucherRepository {
    ...
    public List<Voucher> getList(LocalDate now) {
        String query = """
                       SELECT [idVoucher]
                             ,[Loai]
                             ,[Giatri]
                             ,[HSD]
                             ,[dkNhan]
                         FROM [dbo].[voucher] where ngayKetThuc >= ? order by dkNhan desc
                       """;
        try(Connection con = DBconnect.getConnection()) {
            PreparedStatement ps = con.prepareStatement(query);
            ps.setObject(1, now);
            ...
            List<Voucher> list = new ArrayList<>();
            while(rs.next()){
                Voucher vou = new Voucher(rs.getString(1),rs.getByte(2),rs.getInt(2),rs.getInt(3),rs.getInt(4));
                list.add(vou);
            }
            return list;
        } catch (Exception e) {
